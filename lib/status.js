...
StatusController.prototype.show = function(req, res) {
  ...
  switch(option) {
    ...
    case 'getBlockChainInfo':
      res.jsonp(this.getBlockChainInfo());
      break;
    case 'getNetworkInfo':
    default:
...
StatusController.prototype.getNetworkInfo = function() {
  var info = this.node.services.bitcoind.getNetworkInfo();
  return {
    info: info
  };
};

StatusController.prototype.getBlockChainInfo = function() {
  var info = this.node.services.bitcoind.getBlockChainInfo();
  return {
    chain: chain
  };
};
